...
Uses a mock google-genai client — no real API calls.
...
def _make_fake_result(text: str):
    """Create a fake google-genai GenerateContentResponse with the given text."""
    result = MagicMock()
    result.text = text
...
@pytest.fixture
def state_with_client():
    """BotState with a mock google-genai async client."""
    state = BotState()
    mock_client = MagicMock()
    # generate_content is async — use AsyncMock
    mock_client.aio.models.generate_content = AsyncMock(
        return_value=_make_fake_result("hey, what's up")
    )
    state.ai_client = mock_client
    state.active_model_name = "gemini-1.5-flash-002"
    return state


async def test_generate_response_basic_flow(state_with_client):
    """User message → AI response → appended to history with correct roles."""
    state = state_with_client
    state.ai_client.aio.models.generate_content = AsyncMock(
        return_value=_make_fake_result("hey, what's up")
    )
...
async def test_generate_response_api_none_rolls_back(state_with_client):
    """When API returns None, user turn is removed — no orphaned turn."""
    state = state_with_client
    state.ai_client.aio.models.generate_content = AsyncMock(return_value=None)
...
async def test_generate_response_validation_failure_rolls_back(state_with_client):
    """Prompt-leak response is rejected; user turn is removed."""
    state = state_with_client
    state.ai_client.aio.models.generate_content = AsyncMock(
        return_value=_make_fake_result("as per my dossier i should reply like this")
    )
...
async def test_persona_collapse_rolls_back(state_with_client):
    """Persona collapse (AI admits AI nature) is rejected and rolled back."""
    state = state_with_client
    state.ai_client.aio.models.generate_content = AsyncMock(
        return_value=_make_fake_result("i am an ai and cannot help with that")
    )
...
async def test_opener_injected_for_new_conversation(state_with_client):
    ...
    state.ai_client.aio.models.generate_content = AsyncMock(
        return_value=_make_fake_result("yeah tell me more")
    )
    ...
    assert len(state.sent_openers) == 0  # popped FIFO


async def test_multiple_turns_accumulate_correctly(state_with_client):
...
async def test_sanitization_strips_null_bytes(state_with_client):
    """Null bytes in the user message are stripped before reaching AI."""
    state = state_with_client
    state.ai_client.aio.models.generate_content = AsyncMock(
        return_value=_make_fake_result("ok")
    )
